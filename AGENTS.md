...
- ⚠️ **EF `admin-manage-users` modifiée pour les rôles receptionist/cleaner mais NON redéployée** (nécessite `SUPABASE_ACCESS_TOKEN`). Compte réceptionniste à créer via Admin → Utilisateurs (username requis → login `<username>@staff.local`).
