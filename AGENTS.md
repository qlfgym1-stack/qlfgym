...
- **Auth QLF Gym — Email OTP + Authenticator (sans Google/Apple)** : écran connexion **2 volets** (Admin : Email OTP → code → connexion, + bloc « Connexion avec Authenticator » explicatif ; Réception : username + mot de passe) — boutons Google/Apple retirés de l'UI (providers non configurés, cf. plan §4) ; page Sécurité `/settings/security` refondue en **2 options** (📧 Email OTP toujours actif / 🔐 Authenticator configurable QR-code) avec retrait des blocs Google/Apple ; EF `ensure-super-admin` (secret `SUPER_ADMIN_EMAIL`, JWT requis, appelant = super_admin configuré, appelle RPC `assign_super_admin_role_by_email`) ; clés i18n `auth.connectAuthenticator`/`authenticatorInfo1`/`authenticatorInfo2`/`authenticatorDesc`/`activated` (FR/EN/AR) — pas de migration (tables/RLS déjà ok), pas de nouvelle page super_admin (choix utilisateur) — tsc ✅, vitest ✅ 216/216, build ✅ (132 precache)
- **Auth — bouton « Continuer avec Gmail » (Email OTP)** : volant Admin affiche d'abord un bouton Gmail (logo Google multi-couleurs, clé i18n `auth.continueWithGmail`) qui révèle le champ email → « Recevoir le code » → code OTP → connexion ; retour « ← Utiliser une autre méthode » ; rôle SUPER_ADMIN réellement créé : `qlfgym1@gmail.com` (`SUPER_ADMIN_EMAIL` en secret EF, jamais en dur)
- **EF `ensure-super-admin` — bootstrap jetable** : crée le compte auth.users s'il n'existe pas via header `x-bootstrap-secret` == secret `SUPER_ADMIN_BOOTSTRAP_SECRET` (utilisé une fois puis `supabase secrets unset`), existence vérifiée par boucle paginée `listUsers` (`getUserByEmail` indisponible sur la version esm.sh → TypeError 500) ; chemin normal = JWT du compte SUPER_ADMIN
- **Migration `00117` (appliquée)** : `auto_assign_owner_role` retourne `NEW` quand `auth.uid()` est NULL — sans ça, un RPC service_role créant une organisation (ex. `assign_super_admin_role_by_email`) insérait `user_roles` avec `user_id` NULL (viol NOT NULL, transaction annulée)
...
## Latest (17/09/2026)
- **Déploiement Vercel verrouillé `moussa11` → migration vers compte `qlfgym20@gmail.com`** — le compte Vercel d'origine (`moussa11`, owner `team_j00bYy1OkZ45cXU9ldEGq7qa`, projet `qlf-gym20` `prj_k7sbAUXkRrQ3rSuFTfD5hJNKwqfc`, domaine `qlfgym.vercel.app`) est inaccessible (email de connexion introuvable ; GitHub `qlfgym1-stack` sans email public ; token aminelik limité à l'équipe `infinitygym`). Décision : **nouveau compte propre** pour ne pas saturer les autres projets.
  - **Compte cible** : `qlfgym20@gmail.com` (username `qlfgym20-eng`, user `wJwN7opCSI9nGPVTuWjaERlf`), équipe `qlfgym20-engs-projects` (`team_QuAoW6QHEu2RqYKmPJwG8pK1`). **Token API** : dans `.env.local` + secrets GitHub (jamais commité).
  - **Projet** `fitmanager-pro-dz` (`prj_6EefbM92TX8kvgjLrBlVy2kBy4At`), **vierge**, vars env `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` définies (production/preview/development).
  - **URL production** : https://fitmanager-pro-dz-eight.vercel.app → **HTTP 200** ✅ (déploiement `dpl_EMyXaDVv3ZJ8LALYty1sUncYZ9LY` puis prod `dpl_BA3PCGZWPJRCCYoSXucDs6zdpWXU`, build 229, Version 2.1.10).
  - **Git connect** : liaison natale bloquée (le GitHub App Vercel exige une connexion OAuth sur le compte). Alternative mise en place : **GitHub Actions** — `.github/workflows/deploy-vercel.yml` (build + `vercel deploy --prod` sur push `main`/`master`/`deploy/member-insights`), secrets repo : `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID` créés (libsodium sealed box via API).
  - Souche du repo : `C:\Users\wahra\Desktop\dinatek` (branche `deploy/member-insights`).
- **Site opérationnel ✅ (16h30)** — déploiements Vercel (API `prj_6EefbM92TX8kvgjLrBlVy2kBy4At`) : `dpl_EMyXaDVv3ZJ8LALYty1sUncYZ9LY` (15:32, `89cd1d4` rebranding), `dpl_BA3PCGZWPJRCCYoSXucDs6zdpWXU` (15:36 prod `89cd1d4`), puis GitHub Actions run `35236169762` (15:51, `0fd4b9b` deploy) et run `35240487937`**success** (16:30, `55d9431` fix CORS). Alias prod = `fitmanager-pro-dz-eight` → **HTTP 200** vérifié (routes `/`, `/login`, `/dashboard` toutes 200 ; marque + CSS thème Electric Blue confirmés dans le bundle servi `index-BnyIsJEv.css` : `2563EB`, `38BDF8`, `7C3AED`, `3B82F6`, `8B5CF6`, `080D18`, `F8FAFC`, `card-shadow`).
- **Fix CORS chat IA + fallback 2 clés OpenRouter (commit `55d9431`)** — l'assistant IA renvoyait « Failed to send a request to the Edge Function » car les 8 EFs n'avaient que `https://qlfgym.vercel.app` dans `allowedOrigins` (l'Origin du nouveau domaine était bloquée par CORS, renvoyant `origin: null`). Correctif : **2 domaines ajoutés** (`fitmanager-pro-dz-eight.vercel.app`, `fitmanager-pro-dz.vercel.app`) dans les 8 EFs (ai-chat, admin-manage-users, recovery, sign-in-with-recovery, send-staff-invitation, send-subscription-reminder, send-payment-reminder, create-notification). **Les 8 redéployées** sur Supabase `qgxisgmfnkxwdkchfneb` (versions : ai-chat v14, admin-manage-users v12, recovery v5, sign-in-with-recovery v5, send-staff-invitation v5, send-subscription-reminder v7, send-payment-reminder v7, create-notification v5 ; `verify_jwt` reste **true** partout). Diagnostics : 401 du gateway = JWT manquant/invalide (attendu) ; le CORS de notre code ne s'applique qu'après auth — le blocage navigateur venait bien de l'Origin non whitelistée.
- **Fallback 2 clés OpenRouter dans `ai-chat`** — 2 secrets Supabase : `OPENROUTER_API_KEY` (ancienne `sk-or-v1-eaef…`) + `OPENROUTER_API_KEY_BACKUP` (nouvelle `sk-or-v1-58b2…`) ; même doublette dans `.env` local (gitignoré). `ai-chat/index.ts` : boucle sur les clés — sur **401/403** (clé invalide) bascule sur la clé de secours, sinon reste sur la clé principale (429/404 → modèle suivant). `HTTP-Referer` du fetch passé à `https://fitmanager-pro-dz-eight.vercel.app`.
- **Vérifications** — `npx tsc --noEmit` ✅ zéro erreur ; `npx vitest --run` ✅ **216/216** (22 fichiers) ; `npx vite build` ✅ (128 precache) ; déploiement prod via workflow GitHub Actions automatisé (run success).
- **Thème Electric Blue uniformisé (commit `d64153f`)** — la page login (`sign-in.tsx`) utilisait encore l'ancienne palette (`#054AC2`, `#043da8`, fond `#0a0f1a`) en dur → convertie aux **variables du thème** (`text-primary`, `bg-primary`, `bg-card`, `border-primary/30`, focus `ring-primary/30`) : login, dialog récupération, liens, boutons, code de récupération. Barre rentabilite (`bg-[#10b981]` light / `dark:bg-primary`) → `bg-primary` dégradé primary→secondary. Palette séries `useProfitabilityData.ts` → Electric Blue (`#3B82F6` subscriptions / `#38BDF8` pos / `#8B5CF6` other). **`theme-color` PWA vert `#10b981` → `#2563EB`** (index.html + vite.config manifest, fond `#0a0a0a` → `#080D18`) — sans ça la barre du navigateur mobile restait verte. Les `#10b981` restants dans dashboard/expenses/products sont la couleur sémantique **`--success`** du thème (badges actif, bénéfices) — volontaires. Vérifié en prod : CSS servi `index-CmAMe1vO.css` = hash dist local, `theme-color content="#2563EB"` servi ✅.
- **Bug pointage (commit `b292730`)** — `/pointage` affichait « Something went wrong : m.rpc(...).catch is not a function ». Cause : `pointage.tsx:151` appelait `(supabase.rpc as any)("auto_close_stale_attendances").catch(...)` — en supabase-js, `rpc()` retourne un builder **thenable** (`.then` custom) qui **n'expose pas `.catch`** → TypeError. C'était le **seul** `.rpc(...).catch` du codebase (les autres appels utilisent `await`). Fix : IIFE async + `try/catch` autour de `await`. Prod : CSS servi identique local, `/pointage` → HTTP 200 ✅.
- **Fire-and-forget supprimés (commit `b416e08`)** — 2 derniers builders Supabase chaînés sans gestion de rejet convertis en `await` + `try/catch` : `products.tsx` `handleConfirmImport` (`.from("products").insert(...).then(...)` → async/await, toast d'erreur réseau/RLS) ; `coach-mode.tsx` onClick salaire (`.from('staff').update(...).then(...)` → async/await, toast d'erreur). **Plus aucun `.then()/.catch()` sur un builder `from`/`rpc` dans tout `src`** (vérifié par grep). tsc ✅ · vitest ✅ 216/216 · build ✅.
